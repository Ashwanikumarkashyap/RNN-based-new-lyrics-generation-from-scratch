...
class RnnModel:
    ...
    def back_propagation(self, X, targets):
        ...
        while time_step >= 0:
            # copy the predicted soft-max output
            dy = np.copy(self.output_softmax_prob[time_step])

            # compute loss gradient wrt outputs
            dy[targets[time_step]] = dy[targets[time_step]] - 1

            # compute output to hidden weight gradient wrt loss
            ...
            # compute hidden layer change wrt to loss
            ...
            # compute (1-h^2) * hidden layer change
            ...
            # compute input to hidden wt gradient wrt loss
            ...
            # compute hidden to hidden wt gradient wrt loss
